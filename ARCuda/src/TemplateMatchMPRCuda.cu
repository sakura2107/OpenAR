...
__global__ void templateMatchMPRKernel
	...
	if (x + temp_width <= image_width && y + temp_height <= image_height) {
		...
		for (int i = 0; i < num_points; i++) {
			...
			if (image_x < image_width && image_y < image_height) {
				long long diff = (long long)(image[image_y * image_width + image_x] - temp[temp_y * temp_width + temp_x]);
				sum += diff * diff;
			}
		}
...
bool ar::templateMatchMPRCuda(int* res,
	std::string& msg_res,
	const unsigned char* image,
	const unsigned char* temp,
	...
	const int& num_points)
{
	...
	long long* result_array = new long long[result_size];
	...
	long long* device_result_array = nullptr;
	...
	err = cudaMalloc(&device_result_array, result_size * sizeof(long long));
	if (err != cudaSuccess) {
		delete[] result_array;
		delete[] compare_points_array;
		cudaFree(device_image_array);
	...
	templateMatchMPRKernel << <blocksPerGrid, threadsPerBlock >> > (device_image_array,
		device_temp_array,
		device_result_array,
	...
	err = cudaMemcpy(result_array, device_result_array, result_size * sizeof(long long), cudaMemcpyDeviceToHost);
	if (err != cudaSuccess) {
		delete[] result_array;
		delete[] compare_points_array;
		cudaFree(device_image_array);
